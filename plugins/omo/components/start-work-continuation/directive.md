...
3b. Keep the delivery boundary current-scope only: the active user goal, the first unchecked top-level checkbox, the current diff or staged diff, and ledger entries written for that scope. Historical evidence, obsolete drafts, old review artifacts, abandoned files, and untracked workflow logs are background only unless the active plan explicitly names them or they are inside the current diff/staged diff. Workflow hygiene blocks only when it prevents reproducible verification, corrupts active plan/Boulder state, changes the current commit boundary, exposes sensitive data, or was explicitly requested by the user.
...
Pass the current-scope boundary into review-work and debugging. Review lanes must evaluate the current diff/staged diff, the active plan, and current verification evidence. They must not fail the gate for stale historical evidence, obsolete drafts, old workflow logs, or files outside the current diff/staged diff unless those materials directly contradict the current deliverable, verification, commit boundary, or safety.
